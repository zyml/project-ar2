...
final class AR2_Styles {
	...
	public function __construct() {
		...
		if ( !is_admin() ) {
			...
			add_action( 'ar2_load_styles', array( $this, 'override_styles' ) );
		}

		//add_action( 'wp_head', array( $this, 'override_styles' ) );

		add_filter( 'ar2_custom_styles', array( $this, 'add_logo_css' ) );
	...
	public function default_colors() {
		...
		$this->add_color_set( 'primary', __( 'Primary Color', 'ar2' ), array (
			'bg_selectors'		=> 'input[type=submit]:hover, a.more-link:hover, .navigation a:hover, .comment-controls a:hover, a.post-edit-link:hover, .wp-pagenavi a:hover, .tags a:hover, .tagcloud a:hover, .navigation .current, .wp-pagenavi .current, #main-nav ul.menu ul a:hover, #main-nav ul.menu ul ul a:hover, .multi-sidebar .tabs a:hover, .multi-sidebar .ui-state-active, .multi-sidebar .tabs .ui-state-active a, .posts-node .entry-comments, .posts-quick .entry-comments, .posts-slideshow ul.flex-direction-nav li a:hover, .post-navigation span, .post-navigation a:hover span, .social-nav a:hover',
			'text_selectors'	=> 'a:hover',
			'border_selectors'	=> '#main-nav ul.menu li a:hover, .multi-sidebar .ui-state-active',
		) );
	...
	/**
	 * Overrides the default stylesheet to add a logo if it is available in the theme options.
	 * @since 2.0.1
	 */
	public function add_logo_css( $custom_css ) {
		$logo_options = ar2_get_theme_option( 'logo' );
		$logo_css = '';

		if ( !empty( $logo_options ) && $logo_options[ 'id' ] >= 0 ) {
			if ( $logo_options[ 'height' ] > 120 ) {
				$logo_css = "
.logo {
	padding: " . AR2_LOGO_PADDING_HEIGHT . "px " . AR2_LOGO_PADDING_WIDTH . "px;
}";
			} else {
				$logo_css = "
.logo {
	padding: " . floor( ( 120 - $logo_options[ 'height' ] ) / 2 ) . "px " . AR2_LOGO_PADDING_WIDTH . "px;
}";
			}
...
.blog-name a {
	display: block;
	text-indent: -9000px; 
	background: url(" . ar2_logo_upload_dir_uri() . "logo.png) no-repeat; 
	width: " . $logo_options[ 'width' ] . "px; 
	height: " . $logo_options[ 'height' ] . "px;
	background-size: 100%;
}
.blog-description { 
	display: none; 
...
@media screen and (max-width: 768px) {
	.logo {
		padding: " . AR2_LOGO_PADDING_HEIGHT . "px " . AR2_LOGO_PADDING_WIDTH * 1.5 . "px;
	}
	.blog-name a {
		width: " . $logo_options[ 'width' ] * 1/2 . "px; 
		height: " . $logo_options[ 'height' ] * 1/2 . "px;
	}
	.js #main-nav .toggle {
		margin-top: 15px;
	}
	...
	public function override_styles() {
		...
		foreach ( $this->font_sets as $id => $args ) {
			...
			if ( isset( $this->fonts[ $font_id ] ) && $font_id != 'default' )
				$custom_css .= $this->font_sets[ $id ][ 'selectors' ] . ' { font-family: "' .  $this->fonts[ $font_id ][ 'family' ] . '", "Helvetica Neue", Helvetica, Arial, "Lucida Grande", sans-serif; }' . "\n";
		...
		foreach ( $this->color_sets as $id => $args ) {
			...
			if ( isset( $color ) && $color != 'default' ) {
				if ( isset( $args[ 'bg_selectors' ] ) )
					$custom_css .= $args[ 'bg_selectors' ] . ' { background-color: ' .  $color . '; }' . "\n";
				
				if ( isset( $args[ 'text_selectors' ] ) )	
					$custom_css .= $args[ 'text_selectors' ] . ' { color: ' .  $color . '; }' . "\n";
				
				if ( isset( $args[ 'border_selectors' ] ) )
					$custom_css .= $args[ 'border_selectors' ] . ' { border-color: ' .  $color . '; }' . "\n";
			}
			
		}
		
		// override other styles
		$custom_css = apply_filters( 'ar2_custom_styles', $custom_css );

		// use WP 3.5's wp_add_inline_style() function instead.
		wp_add_inline_style( 'ar2', $custom_css );
 ...
 * Header images are disabled for smartphone layouts (for now).
 * @since 2.0
 */
